...
class DikeNetwork:
    ...
    def __call__(self, timestep=1, **kwargs):
        ...
        for item in kwargs:
            ...
            # the rest of the times you always get a string like {}_{}:
                ...
                if "RfR" in string2:
                    ...
                    #print('cost per RfR', costs_per_RfR)

                    proj_node["cost"] += costs_per_RfR
                    
                    #print('proj_node[cost]', proj_node['cost'])
                    #print(RfR_dict.keys())
                    #print(RfR_dict)
                    if string1 in RfR_dict.keys():
                        RfR_dict[string1] += costs_per_RfR
                    else:
                        RfR_dict[string1] = costs_per_RfR
        ...
        for s in self.planning_steps:
            ...
            #print(G.nodes[f"RfR_projects {s}"]["cost"])
            #print(RfR_dict)
            ...
            #print(RfR_dict)
            for key in RfR_dict:
                new_value = int(RfR_dict[key] / self.num_planning_steps)
                data[f"RfR {key} Individual Costs"].append(new_value)
